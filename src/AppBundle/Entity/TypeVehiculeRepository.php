...
class TypeVehiculeRepository extends EntityRepository
{
    ...
    public function countRowsFiltre($search){
        ...
        $qb->select('count(r.id)')->leftJoin('r.carrosserie', 'c')->leftJoin('r.usage', 'u')->leftJoin('r.genre', 'g')->where('r.libelle like :search or g.libelle like :search or u.libelle like :search OR c.libelle like :search')->setParameter('search', '%'.$search.'%');
        return  $qb->getQuery()->getSingleScalarResult();
    }
